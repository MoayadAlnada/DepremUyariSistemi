...
def kayit(kullanciadi, sifresi):
    ...
    try:
        varmis_data = pd.read_excel('kayitli_kullancilar.xlsx')
        df = pd.concat([varmis_data, df], ignore_index=True)
...
def kayitli_kullancilar():
    try:
        df = pd.read_excel('kayitli_kullancilar.xlsx')
        df.index += 1
        print(df)
    except FileNotFoundError:
        print("kayıtlı bir kullanıcı yok.")
